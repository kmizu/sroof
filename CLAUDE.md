...
**Termination and positivity are checked once each, by the front ends, and the kernel never re-derives them.** `PositivityChecker`/`TerminationChecker` are called from `syntax.Elaborator` and `frontend.CoreTranslator`, nowhere else — so they are as trusted as the kernel with none of its re-checking behind them. v0.35: `TerminationChecker.checkBody` walked a match's *branches* and never its **scrutinee**, so `def toEmpty(n: Nat): Empty { match toEmpty(n) { } }` was accepted — an unguarded recursive call in a match with no branches to walk. That types `Nat -> Empty`, and `sroof check` then reported **OK, exit 0** on a file proving `Nat.zero = Nat.succ(Nat.zero)`. Nothing diverged: the kernel checks the proof term by *type*, and it does have that type. Second hole, same function: the guard accepted a call when *any* argument was structurally smaller, so `match m { case succ(k) => loop(k, m) }` passed — `m` goes back unchanged in the matched position and it loops (StackOverflowError, which is an `Error`, so the `NonFatal` catches added in v0.28 do not see it). The decreasing position is now pinned to the parameter the match takes apart. **When auditing a checker that sits outside the kernel, the question is not "is each case right" but "which sub-terms does it never visit — and at what polarity"** — scrutinee, return type, parameter type annotation, the arguments of an applied inductive. v0.36: `PositivityChecker` visited `App` arguments *at the ambient polarity*, which is only right when the head uses its parameters covariantly — `Neg(A) { mk(f: A -> Empty) }` makes `Neg(Bad)` a function out of `Bad` with no arrow at the occurrence, and `sroof check` certified `0 = 1` a second way, with the termination checker correctly silent (the recursion rides in the data). v0.35 had recorded this as unreachable because the parser rejects arrows inside type applications — an argument about one *spelling*, with no measurement behind it. **Reachability claims about a soundness gate get measured, not argued.** The rule now: `T` in an argument of applied `D` needs the ambient position positive, `T` strictly positive within the argument, and `D` strictly positive in that parameter (recursive, seen-set for cycles; index arguments and unknown inductives rejected conservatively; `Eq` allowed — refl carries no fields). `PositivityChecker.check` takes `(using GlobalEnv)` and `ModuleVerifier` grows the env through its inductive fold in declaration order.

**The kernel guards every `Fix` in a proof term (v0.37).** `Bidirectional.check` types `Fix(f, T, body)` with `f : T` assumed — sound only if the recursion is well-founded, which types cannot see. `Kernel.verify` accepted `Fix("pf", P, Var(0))`, a proof of any `P` by itself (measured), so for `Fix`-shaped proofs — every proof by `induction` — the "sole arbiter" enforced nothing about well-foundedness. `Kernel.check` now runs `TerminationChecker` on every `Fix` node in the proof (each fixpoint guards its own self-reference; nested ones get their own check). Zero false negatives: 741 tests + corpus 26/26 with the guard active. If a tactic's output ever fails it, fix the tactic, never relax the guard.

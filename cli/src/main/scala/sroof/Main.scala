...
import sroof.core.{GlobalEnv, Context}
import sroof.syntax.{Parser, Elaborator, ElabResult, SDecl}
import sroof.tactic.TacticError
import sroof.extract.Extractor

/** Entry point for the sroof CLI.
  *
  * Supports two sub-commands:
  *   - `sroof check <file.sroof>` — parse, elaborate, type-check, and prove
  *   - `sroof repl`                — interactive REPL
  */
object Main:
  ...
  private[sroof] final case class CheckCliOptions(
    filePath: String,
    json: Boolean,
    failOnSorry: Boolean,
  )

  private[sroof] final case class SorryDiagnostic(
    code: String,
    defspec: String,
    transitive: Boolean,
      ...
      rest match
        case Nil =>
          file match
            ...
            case None       => Left("Missing <file.sroof> for check")
        case "--json" :: tail =>
  ...
  private def runAgent(filePath: String): Unit =
    import sroof.agent.{FileRepairer, TacticGen}
    val source =
    ...
    println(s"sroof agent: analysing $filePath ...")
  ...
  private def printUsage(): Unit =
    println(
      """|Usage:
         |  sroof check <file.sroof>                          Parse, elaborate, and verify a sroof file.
  ...
  private[sroof] def parseSorryDiagnostic(warning: String): SorryDiagnostic =
    warning match

...
class SorryPolicySuite extends FunSuite:
  ...
  test("parseCheckOptions: default warning mode without flags"):
    val parsed = Main.parseCheckOptions(List("examples/nat.sroof"))
    assertEquals(
      parsed,
      Right(Main.CheckCliOptions("examples/nat.sroof", json = false, failOnSorry = false)),
    )

  test("parseCheckOptions: accepts --json and --fail-on-sorry in any order"):
    val a = Main.parseCheckOptions(List("--json", "--fail-on-sorry", "f.sroof"))
    val b = Main.parseCheckOptions(List("--fail-on-sorry", "--json", "f.sroof"))
    val expected = Right(Main.CheckCliOptions("f.sroof", json = true, failOnSorry = true))
    assertEquals(a, expected)
    assertEquals(b, expected)

  test("parseCheckOptions: unknown option is rejected"):
    val parsed = Main.parseCheckOptions(List("--unknown", "f.sroof"))
  ...
  test("processSourceJson: fail-on-sorry keeps clean proof successful"):
    val source =
      """|inductive Nat { case zero: Nat  case succ(n: Nat): Nat }
         ...
         |""".stripMargin
    val json = Main.processSourceJson(source, "clean.sroof", failOnSorry = true)

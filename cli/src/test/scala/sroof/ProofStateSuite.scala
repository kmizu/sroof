  ...
  test("failing proof includes (error ...) sexp with tactic message") {
    val source =
      """|inductive Nat {
         ...
         |}
         |""".stripMargin
    val result = Main.processSource(source, "never-proved.sroof")
    assert(result.isLeft)
    val msg = result.left.toOption.get
  ...
  test("context (hyp ...) entries show name and type") {
    ...
    val source =
      """|inductive Nat {
         ...
         |}
         |""".stripMargin
    val result = Main.processSource(source, "two-params.sroof")
    assert(result.isLeft)
    val msg = result.left.toOption.get
  ...
  test("induction succ case shows all hypotheses including ih") {
    val source =
      """|inductive Nat {
         ...
         |}
         |""".stripMargin
    val result = Main.processSource(source, "bad-succ.sroof")
    assert(result.isLeft, s"expected failure but got: $result")
    val msg = result.left.toOption.get
  ...
  test("successful proof does NOT include proof-state sexp in message") {
    val source =
      """|inductive Nat {
         ...
         |}
         |""".stripMargin
    val result = Main.processSource(source, "refl.sroof")
